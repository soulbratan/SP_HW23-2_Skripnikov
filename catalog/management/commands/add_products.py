...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        category_1, _ = Category.objects.get_or_create(
            name="Продуткы", description="Продукты питания"
        )
        category_2, _ = Category.objects.get_or_create(
            name="Одежда", description="Повседневная одежда"
        )

        products_1 = [
            {
                "name": "Помидоры",
                "description": "Помидоры минусинские",
                "price": 330,
                "category": category_1,
            },
            {
        ...
        products_2 = [
            {
                "name": "Джинсы",
                "description": "Джинсовая одежда",
                "price": 3030,
                "category": category_2,
            },
            {
                "name": "Кофты",
                "description": "Вязанные кофты",
                "price": 2600,
                "category": category_2,
            },
        ]
        ...
        for product_data in products_1:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully added product: {products.name}")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f"Product already exist: {products.name}")
                )
